...
esp_err_t deinit_wifi(esp_event_handler_t handler_func){
    ...
    if (ESP_OK != (_err = esp_event_handler_unregister(WIFI_EVENT, ESP_EVENT_ANY_ID, handler_func)) ||
        ESP_OK != (_err = esp_event_handler_unregister(IP_EVENT, IP_EVENT_STA_GOT_IP, handler_func)) ||
